...
#ifdef _DEBUG
#pragma comment( lib, "Box2D/libx86/Ddebug/Box2D.lib" )
#else
...
PhysBody* ModulePhysics::CreateRectangleSensor(int x, int y, int width, int height, b2BodyType body_type)
{
	b2BodyDef body;
	body.type = body_type;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
...
update_status ModulePhysics::PostUpdate()
{
	...
	if (selected_body)
	{
		...
		def.maxForce = 100.0f * selected_body->GetMass();
		mouse_joint = (b2MouseJoint*)world->CreateJoint(&def);
	}
	if (mouse_joint)
	{
		mouse_joint->SetTarget({ PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()) });
		App->renderer->DrawLine(METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().x), METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().y), App->input->GetMouseX(), App->input->GetMouseY(), 255, 100, 100);
